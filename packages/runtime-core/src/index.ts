...
export interface Usage {
  /**
   * Token/cost usage for a completed run.
   *
   * Unified "breakdown" semantics:
   * - `input_tokens`: total input tokens processed for the run (including any cached tokens).
   * - `cache_read_tokens`: subset of `input_tokens` served from a prompt-cache read.
   * - `cache_write_tokens`: subset of `input_tokens` written to a prompt-cache (cache creation).
   * - `output_tokens`: tokens generated by the model.
   * - `total_tokens`: `input_tokens + output_tokens` (cache tokens are not added again).
   * - `context_length`: best-effort estimate of the model context length for the **final model call**
   *   that produced the assistant response (typically `input_tokens + output_tokens` for that call).
   *
   * Providers vary in what they report; fields may be omitted.
   */
  input_tokens?: number;
